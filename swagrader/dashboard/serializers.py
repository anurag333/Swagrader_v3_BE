...
class EntryKeySerializer(serializers.Serializer):
    ...
    def validate_entry_key(self, value):
        if len(value) == 7:
            ...
            raise ValidationError('Ensure this has exactly 7 chars')
...
class AssignmentListCreateSerializer(serializers.ModelSerializer):
    ...
    late_sub_deadline = serializers.DateTimeField(
    ...
    def validate(self, data):
        ...
        if data['submission_deadline'] <= data['publish_date']:
            raise serializers.ValidationError(
                "Submission deadline should not be earlier than publishing date.")

        if data['allow_late_subs']:
            if data['late_sub_deadline'] != None:
                if data['late_sub_deadline'] <= data['submission_deadline']:
                    raise serializers.ValidationError(
                        "Late submission deadline should not be earlier than the submission date.")
                return data
            raise serializers.ValidationError(
                "Set the late submission deadline for the assignment or uncheck the allow late sub flag.")
        return data
...
class CourseDetailInstructorSerializer(serializers.ModelSerializer):
    ...
    authored_assignments = AssignmentListCreateSerializer(
    ...
    class Meta:
        model = Course
        fields = ['instructors', 'course_id', 'course_number', 'course_title', 'entry_key', 'term',
                  'year', 'entry_restricted', 'st_count', 'ta_count', 'course_avg', 'authored_assignments']
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Course
        fields = ['instructors', 'course_id', 'course_number', 'course_title',
                  'term', 'year', 'entry_restricted', 'roles', 'authored_assignments']
...
class SingleUserSerializer(serializers.Serializer):
    ...
    role = serializers.ChoiceField(choices=(
        ('s', 'Student'), ('t', 'Teaching Assistant'), ('i', 'Instructor')), allow_blank=False)
    ...
    def validate_email(self, value):
        ...
        if EmailNamespace.objects.filter(namespace=domain).exists():
            ...
            raise ValidationError(
                'The user should have the institute defined domain namespace.')


class CourseMetadataSerializer(serializers.ModelSerializer):
    class Meta:
        model = CourseMetadata
        fields = ['description', 'grading_policy',
                  'peergrading_policy', 'regrading_policy']
...
class CourseRosterSerializer(serializers.ModelSerializer):
    ...
    def get_role(self, roster):
        role = []
        if roster.user in roster.course.instructors.all():
            role.append('instructor')
        if roster.user in roster.course.teaching_assistants.all():
            role.append('ta')
        if roster.user in roster.course.students.all():
            role.append('student')
        return role
...
class SubQuestionSerializer(WritableNestedModelSerializer):
    ...
    class Meta:
        model = SubQuestion
        fields = ['parent_ques', 'sno', 'sques_id', 'title',
                  'min_marks', 'max_marks', 'global_subrubrics']
...
class PeerGradersSerializer(serializers.ModelSerializer):
    ...
    def get_student(self, peer_graders):
        return peer_graders.student.email

    def get_paper(self, peer_graders):
        return peer_graders.paper.sub_id
